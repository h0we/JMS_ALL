...
Docker Hub [jms_all v3.4.2](https://hub.docker.com/r/h0we/jms_all) All in one 镜像
...
docker run --name jms_all --rm \
  ...
  h0we/jms_all:v3.4.2 init_db   # 确定无报错
```
...
# 例: mysqldump -h192.168.100.11 -p3306 -ujumpserver -pnu4x599Wq7u0Bn8EABh3J91G jumpserver > /opt/jumpserver-v3.4.2.sql

# 拉取新版本镜像
docker pull jumpserver/jms_all:v3.4.2
...
docker run --name jms_all --rm \
  ...
  -e REDIS_PORT=6379 \
  -e REDIS_PASSWORD=****** \
  h0we/jms_all:v3.4.2 upgrade     # 确定无报错
